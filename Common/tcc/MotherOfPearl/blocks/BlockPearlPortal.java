...
import tcc.MotherOfPearl.dimension.world.PearlTeleporter;
import cpw.mods.fml.relauncher.Side;
import cpw.mods.fml.relauncher.SideOnly;
import darkevilmac.MotherOfPearl.lib.Reference;

public class BlockPearlPortal extends BlockBreakable {
    public BlockPearlPortal(int par1) {
        super(par1, Reference.MOD_TEXTURE_ID + "PearlPortal", Material.portal, false);
    ...
    public boolean tryToCreatePortal(World par1World, int par2, int par3, int par4) {
        byte b0 = 0;
        byte b1 = 0;
        if (par1World.getBlockId(par2 - 1, par3, par4) == ModBlocks.pearlstone.blockID || par1World.getBlockId(par2 + 1, par3, par4) == ModBlocks.pearlstone.blockID) {
            b0 = 1;
        }
        if (par1World.getBlockId(par2, par3, par4 - 1) == ModBlocks.pearlstone.blockID || par1World.getBlockId(par2, par3, par4 + 1) == ModBlocks.pearlstone.blockID) {
            b1 = 1;
        }
        if (b0 == b1) {
            return false;
            ...
            for (l = -1; l <= 2; ++l) {
                for (i1 = -1; i1 <= 3; ++i1) {
                    ...
                    if (l != -1 && l != 2 || i1 != -1 && i1 != 3) {
                        ...
                        if (flag) {
                            if (j1 != ModBlocks.pearlstone.blockID) {
                                return false;
                            }
                        } else if (j1 != 0 && j1 != ModBlocks.pearlFire.blockID) {
                            return false;
                        }
                    }
                }
    ...
    public void onNeighborBlockChange(World par1World, int par2, int par3, int par4, int par5) {
        ...
        if (par1World.getBlockId(par2, i1 - 1, par4) != ModBlocks.pearlstone.blockID) {
            ...
            if (j1 == 3 && par1World.getBlockId(par2, i1 + j1, par4) == ModBlocks.pearlstone.blockID) {
                boolean flag = par1World.getBlockId(par2 - 1, par3, par4) == this.blockID || par1World.getBlockId(par2 + 1, par3, par4) == this.blockID;
                boolean flag1 = par1World.getBlockId(par2, par3, par4 - 1) == this.blockID || par1World.getBlockId(par2, par3, par4 + 1) == this.blockID;
                if (flag && flag1) {
                    par1World.setBlockToAir(par2, par3, par4);
                } else {
                    if ((par1World.getBlockId(par2 + b0, par3, par4 + b1) != ModBlocks.pearlstone.blockID || par1World.getBlockId(par2 - b0, par3, par4 - b1) != this.blockID)
                            && (par1World.getBlockId(par2 - b0, par3, par4 - b1) != ModBlocks.pearlstone.blockID || par1World.getBlockId(par2 + b0, par3, par4 + b1) != this.blockID)) {
                        par1World.setBlockToAir(par2, par3, par4);
                    }
                }
            } else {
    ...
    public void onEntityCollidedWithBlock(World par1World, int par2, int par3, int par4, Entity par5Entity) {
        if ((par5Entity.ridingEntity == null) && (par5Entity.riddenByEntity == null) && ((par5Entity instanceof EntityPlayerMP))) {
            ...
            } else if (thePlayer.dimension != Reference.DIMID_1) {
                thePlayer.timeUntilPortal = 10;
                thePlayer.mcServer.getConfigurationManager().transferPlayerToDimension(thePlayer, Reference.DIMID_1, new PearlTeleporter(thePlayer.mcServer.worldServerForDimension(Reference.DIMID_1)));
            } else {
                thePlayer.timeUntilPortal = 10;
                thePlayer.mcServer.getConfigurationManager().transferPlayerToDimension(thePlayer, 0, new PearlTeleporter(thePlayer.mcServer.worldServerForDimension(0)));
            }
        }
    }
